...
half4 GetDisplacement3(half4 tileableUv, half4 tiling, half4 differenceVectorSpeed, sampler2D mapA, sampler2D mapB, sampler2D mapC)
{
	half4 displacementUv = tileableUv * tiling + _Time.xxxx * differenceVectorSpeed;
	#ifdef WATER_VERTEX_DISPLACEMENT_ON			
...
half4 GetDisplacement2(half4 tileableUv, half4 tiling, half4 differenceVectorSpeed, sampler2D mapA, sampler2D mapB)
{
	half4 displacementUv = tileableUv * tiling + _Time.xxxx * differenceVectorSpeed;
	#ifdef WATER_VERTEX_DISPLACEMENT_ON			
		half4 tf = tex2Dlod(mapA, half4(displacementUv.xy, 0.0,0.0));
...
half4  ExtinctColor (half4 baseColor, half extinctionAmount) 
{
	...
	void Gerstner (	out half3 offs, out half3 nrml,
					 ...
					 half4 speed, half4 differenceVectorAB, half4 differenceVectorCD ) 
	{
		#ifdef WATER_VERTEX_DISPLACEMENT_ON
			offs = GerstnerOffset4(tileableVtx.xz, steepness, amplitude, frequency, speed, differenceVectorAB, differenceVectorCD);
			nrml = GerstnerNormal4(tileableVtx.xz + offs.xz, amplitude, frequency, speed, differenceVectorAB, differenceVectorCD);		
		#else
			offs = half3(0,0,0);
			nrml = half3(0,1,0);
		#endif							
